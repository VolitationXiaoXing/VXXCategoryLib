...
- (NSString *)quarter{
    ...
    [dateFormatter setDateFormat:@"qq"];
    return [dateFormatter stringFromDate:self];
}
- (NSInteger)quarterInteger{

    NSDateComponents *dateComponents = [[NSCalendar currentCalendar] components:NSYearCalendarUnit|NSMonthCalendarUnit|NSDayCalendarUnit|NSHourCalendarUnit|NSMinuteCalendarUnit|NSCalendarUnitQuarter fromDate:self];
...
- (NSString *)currentWeekBeginTimeAndEndTime{
    ...
    BOOL ok = [calendar rangeOfUnit:NSWeekCalendarUnit startDate:&beginDate interval:&interval forDate:self];
    //分别修改为 NSMonthCalendarUnit NSDayCalendarUnit NSWeekCalendarUnit NSYearCalendarUnit
    if (ok) {
...
- (NSString *)currentWeekBeginTimeAndEndTimeWithDateFormatterString:(NSString *)dateFormatterString{
    ...
    NSCalendar *calendar = [NSCalendar currentCalendar];
    [calendar setFirstWeekday:2];//设定周一为周首日
    BOOL ok = [calendar rangeOfUnit:NSCalendarUnitWeekOfYear startDate:&beginDate interval:&interval forDate:self];
    //分别修改为 NSMonthCalendarUnit NSDayCalendarUnit NSWeekCalendarUnit NSYearCalendarUnit
    if (ok) {
    ...
    NSDateFormatter *myDateFormatter = [[NSDateFormatter alloc] init];
    [myDateFormatter setDateFormat:dateFormatterString];
    NSString *beginString = [myDateFormatter stringFromDate:beginDate];
    NSString *endString = [myDateFormatter stringFromDate:endDate];
...
- (NSString *)currentYearBeginTimeAndEndTime{
    ...
    NSCalendar *calendar = [NSCalendar currentCalendar];
    [calendar setFirstWeekday:2];//设定周一为周首日
    BOOL ok = [calendar rangeOfUnit:NSCalendarUnitYear startDate:&beginDate interval:&interval forDate:self];
    if (ok) {
...
- (NSString *)weekOfYear;{
    ...
    NSCalendar *calendar =
    [[NSCalendar alloc]initWithCalendarIdentifier:NSCalendarIdentifierISO8601]; //ISO08601格式日历
...
- (NSString *)yearWeekStartEndWeekOfYear{
    ...
    NSString *year = [self currentWeekBeginTimeAndEndTimeWithDateFormatterString:@"yyyy.MM.dd"];
    NSString *weekOfYear = [self weekOfYear];
    
    return [year stringByAppendingFormat:@"(第%@周)",weekOfYear];
}

- (NSString *)yearWithWeekOfYear{

    NSDateFormatter *dateFormatter = [[NSDateFormatter alloc]init];
    [dateFormatter setDateFormat:@"yyyyw"];
    
    NSCalendar *calendar =
    [[NSCalendar alloc]initWithCalendarIdentifier:NSCalendarIdentifierISO8601];
    [dateFormatter setCalendar:calendar];
    return [dateFormatter stringFromDate:self];
}
...
- (NSString *)yearWithQuarterOfYear{
    ...
    NSDateFormatter *dateFormatter = [[NSDateFormatter alloc]init];
    [dateFormatter setDateFormat:@"yyyyqq"];
    return [dateFormatter stringFromDate:self];
}
...
- (NSString *)stringYearMonthDayHourMinuteSecondMsec{
...
- (NSString *)stringYearMonthDayHourMinuteSecondMsecWithoutSybol{

    NSDateFormatter *formatter = [[NSDateFormatter alloc]init];
    [formatter setDateFormat:@"yyMMddHHmmssSSS"];
    NSString *str = [formatter stringFromDate:self];
//    NSString *newDateStr = [str stringByReplacingOccurrencesOfString:@" " withString:@""];
//    NSString *aNewDateStr = [newDateStr stringByReplacingOccurrencesOfString:@"-" withString:@""];
//    NSString *needDateStr = [aNewDateStr stringByReplacingOccurrencesOfString:@":" withString:@""];
    return str;
}
